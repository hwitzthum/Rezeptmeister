...
const bodySchema = z.object({
  ...
  original_servings: z.number().int().min(1).max(1000),
  target_servings: z.number().int().min(1).max(1000),
});

export async function POST(request: Request) {
  const ip = getClientIp(request);
  const rl = await checkRateLimitDistributed(`scale-recipe:${ip}`, AI_LIMIT);
  if (!rl.allowed) {
  ...
  const backendRes = await fetchBackendWithRetry("/ai/scale-recipe", {
    method: "POST",
  ...
  if (!backendRes.ok) {
    let detail = "Rezept konnte nicht skaliert werden.";
    try {
