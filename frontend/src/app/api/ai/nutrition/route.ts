...
export async function POST(request: Request) {
  ...
  const rl = await checkRateLimit(`nutrition-calc:${ip}`, AI_LIMIT);
  if (!rl.allowed) {
  ...
  const backendRes = await fetchBackendWithRetry("/ai/nutrition", {
    method: "POST",
  ...
  if (!backendRes.ok) {
    let detail = "Nährwertberechnung fehlgeschlagen.";
    try {
