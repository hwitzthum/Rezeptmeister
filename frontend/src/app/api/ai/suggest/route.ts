...
export async function POST(request: Request) {
  ...
  const rl = await checkRateLimit(`suggest:${ip}`, AI_LIMIT);
  if (!rl.allowed) {
  ...
  const backendRes = await fetchBackendWithRetry("/ai/suggest", {
    method: "POST",
  ...
  if (!backendRes.ok) {
    let detail = "Rezeptvorschläge konnten nicht geladen werden.";
    try {
