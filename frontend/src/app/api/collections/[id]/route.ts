...
export async function GET(
  ...
  const rl = await checkRateLimit(`collections-detail:${ip}`);
  if (!rl.allowed) {
...
export async function PUT(
  ...
  const ip = getClientIp(request);
  const rl = await checkRateLimit(`collections-update:${ip}`);
  if (!rl.allowed) {
    return NextResponse.json({ error: "Zu viele Anfragen." }, { status: 429 });
  }
...
export async function DELETE(
  ...
  const ip = getClientIp(request);
  const rl = await checkRateLimit(`collections-delete:${ip}`);
  if (!rl.allowed) {
    return NextResponse.json({ error: "Zu viele Anfragen." }, { status: 429 });
  }
