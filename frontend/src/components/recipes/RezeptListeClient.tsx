...
interface RezeptListeClientProps {
  initialItems: RecipeListItem[];
  initialTotal: number;
  initialHasMore: boolean;
}

export default function RezeptListeClient({
  initialItems,
  initialTotal,
  initialHasMore,
}: RezeptListeClientProps) {
  ...
  // Daten-State — initial server-seitig gerendert (kein Lade-Flash beim Erstaufruf)
  ...
  // Neu laden wenn Filter sich ändern. Der erste Render nutzt die server-seitig
  // gelieferten Daten, daher wird der initiale Effekt-Lauf übersprungen.
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[1, 2, 3, 4, 5, 6].map((n) => (
              <div
                key={n}
                className="rounded-2xl overflow-hidden border border-[var(--border-base)]"
              >
          ...
          <EmptyState
            hasFilters={
              !!(q || kategorie || kueche || schwierigkeit || nurFavoriten)
            }
          />
        ) : (
          <>
